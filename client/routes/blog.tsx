...
export const Route = createFileRoute("/blog")({
  component: () => {
    ...
    const [imageUploaded, setImageUploaded] = useState(false); // Track if an image is uploaded
    ...
    const handleImageSave = () => {
      // After clicking save, switch the button back to the upload state
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        const imageURL = URL.createObjectURL(file); // Temporarily create image URL
        setImageUrl(imageURL); // Save image URL to state
        setImageUploaded(true); // Image has been uploaded
      }
    };

    // Handle image removal
    const handleImageRemove = () => {
      setImageUrl(null); // Remove image
      setImageUploaded(false); // Reset to upload state
    };
        ...
        <div
          ...
          style={{
            backgroundImage: imageUrl
              ? `url(${imageUrl})`
              : "url('default-image-url')", // Fallback to default image if none is uploaded
          }}
        >
          ...
          {/* Cancel button in the top-left corner with black outline */}
          {imageUrl && (
            <button
          ...
          {!imageUploaded ? (
            ...
            >
              <FontAwesomeIcon icon={faUpload} className="text-black" />
            </label>
          ) : (
            <button
