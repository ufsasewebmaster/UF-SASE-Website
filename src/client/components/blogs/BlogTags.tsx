...
const BlogTags: React.FC<BlogTagsProps> = ({ activeTag, onSearch, onTagClick, tags }) => {
  ...
  const visibleTags = tagArray.slice(
    currentPage * TAGS_PER_PAGE, 
    currentPage * TAGS_PER_PAGE + TAGS_PER_PAGE
  );
  ...
  const tagButtonClasses = (isActive: boolean) => cn(
            ...
            <>
              ...
              {visibleTags.map((tag, index) => (
                <React.Fragment key={index}>
                  <button
                    className={tagButtonClasses(activeTag === tag)}
